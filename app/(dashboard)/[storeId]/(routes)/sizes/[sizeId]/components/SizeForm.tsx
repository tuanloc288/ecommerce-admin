...
const SizeForm: FC<SizeFormProps> = ({
    ...
    const onSubmit = async (data: SizeFormValue) => {
        try {
            setLoading(true)
            // if existed => call api to update size for a specific store
            // if not existed => call api to create new size for a specific store 
            if(initialData){
    ...
    const onDelete = async () => {
        try {
            ...
            toast.error('Make sure that you already removed all products using this size')
        } finally {
            setLoading(false)
            setOpen(false)
        }
